...
def mp3_to_numpy(language):
    ...
    audio_list = np.zeros((len(os.listdir("./data/{}/".format(language))), int(max_len / 5))) #divided by 10 here
    count = 0
    for file in os.listdir("./data/{}/".format(language)):
        try:
            audio = AudioSegment.from_mp3("./data/{}/".format(language) + file)
            ...
            continue
        audio_np_short = audio_np[int(len(audio_np) / 5):2*int(len(audio_np) / 5)] #added this
